...
class _VoizeEmotionSpectrumStartPagesWidgetState
  ...
  /// 自动验证并跳转
  Future<void> _autoVerifyAndJump() async {
    if (!mounted) return;
    PianoLoading.show(context, message: 'Loading now...');
    final startTime = DateTime.now();
    const minLoadingDuration = Duration(milliseconds: 1500);

    try {
      final controller = VerificationController();
      final h5Url = await controller.startVerification();
      ...
      if (h5Url != null && h5Url.isNotEmpty) {
        // 关闭 Loading
